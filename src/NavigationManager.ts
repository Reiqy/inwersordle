export class NavigationManager {
  ...
  selectByIndex(row: number, column: number): void {
    ...
    let cell = document.querySelectorAll("div.row")[row].children[
      column
    ] as HTMLElement;
  ...
  selectPreviousCell(
    moveBetweenRows: boolean = false,
    moveDeletionSelect: boolean = false
  ): void {
  ...
  getNextCell(cell: HTMLElement, moveBetweenRows: boolean = false) {
    ...
    if (moveBetweenRows && nextSibling == null) {
      ...
      if (cell.parentElement.nextElementSibling)
        return cell.parentElement.nextElementSibling
  ...
  selectAdjacentRow(previousRow: boolean = false): void {
    ...
    if (previousRow) {
      adjacentCell =
        this.inputPosition.parentElement.previousElementSibling?.querySelectorAll(
          "div"
        )[i];
    } else {
      adjacentCell =
        this.inputPosition.parentElement.nextElementSibling?.querySelectorAll(
          "div"
        )[i];
    }
  ...
  selectCell(
    ...
    if (selectedCell == null || selectedCell.classList.contains("disabled")) {
      return;
    }
